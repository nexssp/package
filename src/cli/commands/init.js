module.exports = (cmd, args) => {
  ...
  if (pkg) {
    if (!fs.existsSync(`${process.env.NEXSS_PACKAGES_PATH}/${pkg}`)) {
      console.error(
        `Package ${red(pkg)} not found. To install new packages eg: ${green(
          bold(` nexss pkg install ${pkg}`)
        )}`
      )
      process.exitCode = 1
      return
          ...
          if (!fs.existsSync(`${packagesPath}/${author}/${pkg}/_nexss.yml`)) {
            if (fs.statSync(`${packagesPath}/${author}/${pkg}`).isDirectory()) {
              fs.readdirSync(`${packagesPath}/${author}/${pkg}`).map((details) => {
                console.log(`${blue('Package setup:')} ${packagesPath}/${author}/${pkg}`)
                try {
